...
For Docker and Podman scaffolds, Sandcastle keeps Codex subscription auth project-local. Run this once after `sandcastle init`:

```bash
CODEX_HOME="$PWD/.sandcastle/codex-home" codex login
```

The generated sandbox provider sets `CODEX_HOME=/home/agent/.codex` and mounts `.sandcastle/codex-home/` there. Isolated or cloud sandbox providers should use environment-based auth such as `OPENAI_API_KEY`.
